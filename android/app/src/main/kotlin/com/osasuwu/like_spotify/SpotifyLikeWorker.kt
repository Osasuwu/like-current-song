...
class SpotifyLikeWorker(
    ...
    private fun saveToLibrary(
        ...
        if (!SpotifyLibraryEndpoints.useLegacyEndpoints) {
            ...
            val connection = api(SpotifyLibraryEndpoints.libraryUrl(uri), token, "PUT")
            val code = connection.responseCode
    ...
    /**
     * [errorBody] carries Spotify's own error payload on a failed write, so a
     * log line can say *why* instead of only how many.
     */
